...
class Comment extends Component {
  ...
  handleCommentVote = (comment, option) => {
    this.props.vote(comment.id, option);
  };
    ...
    return (
      <div>
        <Card style={{ padding: 5, margin: 5 }}>
          ...
          />
          <CardText style={{ fontSize: '1.2em' }}>{comment.body}</CardText>
          <CardText>Vote Score: {comment.voteScore}</CardText>
          <CardActions>
            <IconButton tooltip="Edit" onClick={this.toggleCommentModal}>
              {' '}
              <Edit />{' '}
            </IconButton>
            <IconButton
            ...
            <IconButton
              tooltip="Vote Up"
              onClick={() => this.handleCommentVote(comment, VOTE_UP)}
            >
              {' '}
              <ActionThumbUp />{' '}
            </IconButton>
            <IconButton
              tooltip="Vote Down"
              onClick={() => this.handleCommentVote(comment, VOTE_DOWN)}
            >
              {' '}
              <ActionThumbDown />{' '}
            </IconButton>
